...
namespace UI.Stats
{
    public class StatShowUI : UIElement
    {
        ...
        public override void OnCreate(InteractorsBase interactorsBase)
        {
            ...
            _itemEquipper = interactorsBase.GetInteractor<PlayerInteractor>().MainPlayer.ItemEquipper;
            CreateStats();

            _statsValueStorage.OnStatsChange += CreateStats;
            _itemEquipper.OnStatModified += CreateStats;
        }
        ...
        //    stringBuilder.Append("Mana Regeneration: ").Append(_statsFinder.GetStat(Stat.ManaRegeneration)).Append($"<color=#00D100>" +
            ...
            stringBuilder.Append("Attack Speed: ").Append(_statsFinder.GetStat(Stat.AttackSpeed)).Append($"<color=#00D100>" +
                $" {_statsValueStorage.GetCalculatedStat(Stat.AttackSpeed)}/" + 
                $"{_statsFinder.GetStat(Stat.AttackSpeed) - _statsValueStorage.GetCalculatedStat(Stat.AttackSpeed)}</color>").AppendLine();
